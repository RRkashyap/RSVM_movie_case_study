...
/* -- Query Output - Find below table wise row columns details which we got from above query
...
-- Query Output - As we can see, total 4 columns has null values (country has 20, worlwide_gross_income has 3724, language has 194 & production_compney has 528 null values in movie table)
...
-- Query Output In 2017 - 3052 movies, 2018 - 2944 movies and 2019 - 2001 movies were released

-- Month wise movies release count
SELECT 
...
/* -- Query Output - Month wise movies released output as followed
...
-- Query Output - India or USA produced total 1059 movies in year 2019
...
SELECT 
    genre, COUNT(*) as count_of_movies
FROM
    genre
GROUP BY genre;
-- Query Output - Total 36 unique genre is present in genre table

SELECT 
    year, genre, COUNT(genre) as count_of_movie
FROM
    genre AS g
        JOIN
...
-- Query Output - We can see that Drama genre has most number of count of movies for last 3 years
...
-- Query output - Drama genre has max number of movie count which is 4285
...
select sum(count) as count_of_movie_belongs_to_one_genre
from temp;

-- Query output - Total 14662 records are there in Genre table wherein movies that belong to only one genre is 3289
...
/* Query output - output is as followed
genre	avg_duration	
Action	113
Romance	110
Drama	107
...
-- query output - Thriller category has 3 number rank in movie count 
...
/* Output format:
+---------------+-------------------+---------------------+----------------------+-----------------+-----------------+
| min_avg_rating|	max_avg_rating	|	min_total_votes   |	max_tota
'9;.8\
116+5+l_votes 	 |min_median_rating|min_median_rating|
+---------------+-------------------+---------------------+----------------------+-----------------+-----------------+
|		0		|			5		|	       177		  |	   2000	    		 |		0	       |	8			 |
+---------------+-------------------+---------------------+----------------------+-----------------+-----------------+*/
-- Type your code below:
...
-- 24 Drama genre movie were released during march 2017 in the USA and had more than 1,000/- votes
-- Top 3 genres during March 2017 in  USA and had more than 1,000/- votes are Drama,Comedy and action
...
with temp as
        (SELECT 
            genre
        FROM
            genre g join ratings r using (movie_id)
		where
			avg_rating > 8
        GROUP BY genre
        ...
        LIMIT 3)
SELECT 
    name AS director_name, COUNT(name) AS movie_count
FROM
    movie AS m
        JOIN
    ratings r ON m.id = r.movie_id
        JOIN
    director_mapping AS d ON d.movie_id = r.movie_id
        JOIN
...
from
	movie as m join ratings as r on m.id=r.movie_id
group by Production_company
...
From
	movie m join role_mapping rm on m.id=rm.movie_id
...
avg_inter_days as 
(
  select name_id,round(AVG(diff),0) as avg_inter_movie_days
